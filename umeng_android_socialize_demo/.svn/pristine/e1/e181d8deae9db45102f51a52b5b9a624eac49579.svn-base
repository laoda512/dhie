...
public class CommentFragment extends Fragment{
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View root = inflater.inflate(R.layout.umeng_example_socialize_commentmod_example,
				container, false);
		final Context mContext = getActivity();
		
		final String commentStr = getResources().getString(R.string.umeng_socialize_share_content);
//		controller.getConfig().setSsoHandler(new QZoneSsoHandler(getActivity()));
		controller.getConfig().setSsoHandler(new SinaSsoHandler());
		controller.getConfig().setSsoHandler(new TencentWBSsoHandler());
		//评论列表页
		root.findViewById(R.id.comment_list).setOnClickListener(new OnClickListener() {
			@Override
				...
				new OnClickListener() {
					...
					public void onClick(View v) {
						...
						if(UMInfoAgent.isLogin(mContext)){
							...
							controller.showLoginDialog(mContext, new LoginListener() {

								@Override
								public void loginSuccessed(SHARE_MEDIA platform,
										boolean noConnect) {
									super.loginSuccessed(platform, noConnect);
					...
					private void sentComment(final Context mContext,
							...
							controller.postComment(mContext,socializeComment, new MulStatusListener() {
									@Override
									public void onStart() {
		...
		root.findViewById(R.id.interface_getComments).setOnClickListener(
				new OnClickListener() {
					@Override
					public void onClick(View v) {
						controller.getComments(getActivity(), new FetchCommetsListener() {
							@Override
							public void onStart() {
							}
							...
							public void onComplete(int status, List<UMComment> comments,SocializeEntity entity) {
								if(status == 200 && comments != null){
									...
									Toast.makeText(mContext, "已获取评论 count="+comments.size(), 1).show();
								}
							}
