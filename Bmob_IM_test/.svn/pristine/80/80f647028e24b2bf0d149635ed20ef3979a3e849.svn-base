...
public class MyMessageReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		BmobLog.i("listener num = " + ehList.size());
		userManager = BmobUserManager.getInstance(context);
		currentUser = userManager.getCurrentUser();
		boolean isNetConnected = NetUtil.isNetworkAvailable(context);
		if(isNetConnected){
...
//			//增加消息接收方的ObjectId--目的是解决多账户登陆同一设备时，无法接到到非当前登陆用户的消息。
			...
			if(!BmobDB.create(context,toId).isBlackUser(username)){//该消息发送方不为黑名单用户
				if(TextUtils.isEmpty(tag)){//不携带tag标签
					...
					}else if(tag.equals(BmobConfig.TAG_OFFLINE)){//下线通知
						if (ehList.size() > 0) {// 有监听的时候，传递下去
							for (EventListener handler : ehList)
								handler.onOffline();
						}else{
