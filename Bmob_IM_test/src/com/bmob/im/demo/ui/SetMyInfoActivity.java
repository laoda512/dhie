...
public class SetMyInfoActivity extends ActivityBase implements OnClickListener{
	...
	private void initMeData() {
		...
		if(BmobUserManager.getInstance(this).getCurrentUser()!=null){
			tv_set_name.setText(BmobUserManager.getInstance(this).getCurrentUser().getUsername());
		}
		tv_set_nick.setText(mApplication.getNick());
		tv_set_gender.setText(mApplication.getSex() == true ? "男" : "女");
	}
	...
	private void refreshAvatar(){
		String avatar =mApplication.getUserAvatar();
		if (avatar != null && !avatar.equals("")) {
			ImageLoader.getInstance().displayImage(avatar, iv_set_avator, ImageLoadOptions.getOptions());
		} else {
	...
	private void uploadAvatar(){
		...
		bmobFile.upload(this, new UploadFileListener() {
			...
			public void onSuccess() {
				...
				//设置头像
				mApplication.setUserAvatar(url);
				//更新头像
				refreshAvatar();
				//更新BmobUser对象
